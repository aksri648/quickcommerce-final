...
export const CreateStoreSchema = z.object({
  ...
  openingTime: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/, 'Format must be HH:mm (e.g. 07:00)'),
  closingTime: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/, 'Format must be HH:mm (e.g. 23:00)'),
...
export const CreateProductSchema = z.object({
  ...
  unit: z.string().min(1).max(50), // e.g. "500 g", "1 L", "Pack of 2"
...
export const UpdateSlotCapacitySchema = z.object({
  ...
  bookingCutoffMinutes: z.number().int().min(0).optional(),
...
export const CheckoutOrderSchema = z.object({
  ...
  deliveryDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date format must be YYYY-MM-DD'),
...
export const CreateDeliveryBatchSchema = z.object({
  storeId: z.string().uuid(),
  deliverySlotId: z.string().uuid(),
  orderIds: z.array(z.string().uuid()).min(1, 'At least one order required to create a batch'),
});
...
export const CreateDriverSchema = z.object({
  ...
  storeId: z.string().uuid(),
  vehicleType: z.string().min(2), // e.g. "Two Wheeler", "Electric Scooter"
